    ...
    public void functionENTROPY()
    {
        ...
        if (refresh == ConfigHandler.effectENTROPYRefreshTime)
        {
            for (EntityPlayer living : entityPlayer)
            {
                ...
                {
                    ItemStack[] inventory = living.inventory.mainInventory;
                    ...
                    {
                        living.inventory.mainInventory[x] = list.get(x);
                    }
        ...
        if (refresh == ConfigHandler.effectWEATHERRefreshTime)
        {
            for (EntityLivingBase living : entityLiving)
            {
                if (living != null)
                {
                    ...
                    worldObj.spawnEntityInWorld(new EntityLightningBolt(worldObj, living.posX, living.posY, living.posZ));
                    refresh = 0;
                    ticksTillRefresh--;
                }
            }
